...
class PatternMgr:
    ...
    def __init__(self):
        ...
        self._botName = u"P8-BOT"
        punctuation = "\"`~!@#$%^&*()_=+[{]}\|;:',<>/?"     # removed . - symbols to make words such as C.O.E, etc. applicable
    ...
    def star(self, starType, pattern, that, topic, index):
        ...
        patMatch, template = self._match(input_.split(), thatInput.split(), topicInput.split(), self._root)
        if template == None:
            return "Sorry I am still learning. I have no idea what are you talking about"
