...
def login(user=False):
	...
	conn = sqlite3.connect('C:/Users/Jehanvai/Desktop/pythonsqlite.db')
	c=conn.cursor()
...
def matchingSentence(inp):
	conn = sqlite3.connect('C:/Users/Jehanvai/Desktop/pythonsqlite.db')
	c = conn.cursor()
	f = open('C:/Python/Python37-32/MBSTU CHATBOT/database/questions.txt')
	match = "";
	max_score=0;
		...
		if(len(inp.split(" "))==1):
			if(isKeyword(inp)==False):
				...
				if(PWD_REQ==1):
					...
					c.close()
					conn.close() 
					return "I'll now be able to answer about MBSTU related queries if your credentials are valid! Otherwise you will have to provide your credentials again!"
				c.close()
				conn.close() 
